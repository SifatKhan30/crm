                                        ...
                                        <div class="com-md-7">
                                            <form action=""  class="form-inline">
                                                <select name="role_id" id="admin_select" class="form-control">
                                                    ...
                                                    <?php
                                                    $admin = $con->query("SELECT * FROM admin WHERE role='admin'");
                                ...
                                <div class="card-body">
                                    <table class="table table-striped table-hover table-bordered">
                                        ...
                                        <?php
                                            $pay_total=0;
                                            $tar_total=0;
                                            $ad_id=$con->query("SELECT admin.name, admin_target.admin_id, admin_target.amount, admin_target.target_month FROM admin JOIN admin_target ON admin_target.admin_id=admin.id");
                                            while($d=$ad_id->fetch_assoc()){
                                                $mark_id=$con->query("SELECT id, name AS mark_name FROM admin WHERE parent=".$d['admin_id']);
                                                $tar_total=$tar_total+$d['amount'];
                                                while($m=$mark_id->fetch_assoc()){
                                                    $deal_id=$con->query("SELECT id, name AS deal_name FROM admin WHERE parent=".$m['id']);
                                                    while($dl=$deal_id->fetch_assoc()){
                                                        $inv=$con->query("SELECT payable, created_at FROM invoice WHERE dealer_id=".$dl['id']);
                                                        while($in=$inv->fetch_assoc()){
                                                            $pay_total=$pay_total+$in['payable'];               
                                                ...
                                                <td><?php echo $tar_total ?></td>
                                                <td><?php echo $pay_total ?></td>
                                                <td><?php echo (($pay_total < $tar_total )?$pay_total/$tar_total*100: '100') ?></td>
                                            </tr>
                                            <?php 
                                            }
                                        ?>
